...
class MultiRadioButtons(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._show_advanced_label: Optional[ctk.CTkLabel] = None
    ...
    def _create_radio_buttons(self) -> None:
        ...
        for index, (item_key, item_config) in enumerate(self._items.items()):
            ...
            radio_button = self._create_radio_button(target_frame, item_config, item_key)
            radio_button.grid(ipady=self.GRID_PADDING, row=index, column=0, sticky="nwe")
            self._radio_buttons[item_key] = radio_button
            
            # Create associated label (error or description)
            self._create_associated_label(target_frame, item_config, item_key, index)
    ...
    def _create_description_label(self, parent, description_text: str, row: int) -> None:
    ...
    def set_selected_value(self, value: str) -> None:
        ...
        if value in self._items:
            ...
            raise ValueError(f"Invalid value: {value}. Must be one of: {list(self._items.keys())}")
