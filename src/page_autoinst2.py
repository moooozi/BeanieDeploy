...
def run():
    ...
    check_wifi.grid(ipady=5, row=0, column=0, sticky=GV.UI.DI_VAR['w'])
    ...
    check_encrypt.grid(ipady=5, row=1, column=0, sticky=GV.UI.DI_VAR['w'])
    ...
    def show_encrypt_options(var):
        if var.get():
            frame_encryption_options.grid(ipady=5,padx=(30, 0), row=2, column=0, sticky=GV.UI.DI_VAR['w'])
        else:
