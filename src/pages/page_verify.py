...
def build_review_text(selected_spin, kickstart, install_options):
    ...
    if (
        kickstart
        and getattr(kickstart, "partitioning", None)
        and kickstart.partitioning.method == PartitioningMethod.CUSTOM
    ):
        ...
        if (
            kickstart
            and getattr(kickstart, "partitioning", None)
            and kickstart.partitioning.method == PartitioningMethod.DUALBOOT
        ):
            review_sel.append(
                _("verify.autoinst.dualboot") % {"distro_name": selected_spin.name}
            )
            review_sel.append(_("verify.autoinst.keep.data"))
        elif (
            kickstart
            and getattr(kickstart, "partitioning", None)
            and kickstart.partitioning.method == PartitioningMethod.REPLACE_WIN
        ):
            review_sel.append(
                _("verify.autoinst.replace.win") % {"distro_name": selected_spin.name}
            )
            review_sel.append(_("verify.autoinst.rm.all"))
        if install_options and getattr(install_options, "export_wifi", False):
            review_sel.append(
                _("verify.autoinst.wifi") % {"distro_name": selected_spin.name}
            )
    return review_sel
