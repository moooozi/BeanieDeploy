...
def run():
    ...
    if GV.IP_LOCALE:
        ...
        if locale_from_ip != GV.KICKSTART.lang:
            tkt.add_radio_btn(page_frame, LN.keymap_tz_option % locale_from_ip_name, keymap_timezone_source_var,
                              'ip', command=lambda: spawn_more_widgets())

    tkt.add_radio_btn(page_frame, LN.keymap_tz_option % chosen_locale_name, keymap_timezone_source_var, 'select',
                      lambda: spawn_more_widgets())
    tkt.add_radio_btn(page_frame, LN.keymap_tz_custom, keymap_timezone_source_var, 'custom',
                      lambda: spawn_more_widgets())
